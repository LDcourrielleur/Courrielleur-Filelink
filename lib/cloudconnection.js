...
class CloudConnection {
    ...
    constructor(accountId) {
        console.log(`[Pacome Debug] CloudConnection constructor for ${accountId}`);
     ...
     */
    async store() {
        // [Pacome Integration] Security: Do NOT store the password in localStorage.
        // Create a shallow copy to avoid modifying the active in-memory object.
        const toStore = Object.assign({}, this);
        delete toStore.password;

        browser.storage.local.set({ [this._accountId]: toStore, });
    }
    ...
    */
    async load() {
        console.log(`[Pacome Debug] CloudConnection.load() for ${this._accountId}`);
        ...
        console.log(`[Pacome Debug] CloudConnection loaded for user: ${this.username}`);
        console.log(`[Pacome Debug] checking browser.pacomeLogin: ${typeof browser.pacomeLogin}`);

        // [Pacome Integration] Try to fetch password from secure storage
        if (this.username && browser.pacomeLogin) {
            console.log("Pacome Integration (CloudConnection): Checking stored password for user " + this.username);
            try {
                const pacomePwd = await browser.pacomeLogin.getPassword(this.username);
                if (pacomePwd) {
                    console.log("Pacome Integration (CloudConnection): Password found in Pacome storage. Using it.");
                    this.password = pacomePwd;
                } else {
                    console.log("Pacome Integration (CloudConnection): No password returned by Pacome.");
                }
            } catch (e) {
                console.error("CloudConnection: Failed to fetch Pacome password", e);
            }
        }
