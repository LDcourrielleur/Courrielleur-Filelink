...
class Status {
    ...
    /**
     * Set the upload status to error state with a specific reason key
     * The reason is used in progress.js to display a more specific error message
     * instead of combining status_error + status_<status>.
     * @param {string} reasonKey A locale message key (without the 'status_' prefix)
     */
    async failWithReason(reasonKey) {
        this.errorReason = reasonKey;
        await this.fail();
    }
